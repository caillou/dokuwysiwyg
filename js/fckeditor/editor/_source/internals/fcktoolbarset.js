...
FCKToolbarSet.prototype.RefreshItemsState = function( editorInstance )
{
    ...
    var bInTable = FCK.EditorDocument != null && FCKSelection.HasAncestorNode( 'TABLE' );

	for ( var i = 0 ; i < aItems.length ; i++ ) {
	    if (bInTable && aItems[i].CommandName == 'FontFormat') {
            // disable FIleFormat from here if inside a table
            aItems[i].Disable();
        } else
		aItems[i].RefreshState() ;
